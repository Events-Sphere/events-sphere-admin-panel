    ...
    <div className="h-[100vh] overflow-x-hidden p-4 container bg-[url('../../public/bg.jpg')] bg-no-repeat bg-cover">
      <table className="w-full bg-white border border-grey shadow-lg rounded-lg overflow-hidden">
        ...
        <tbody>
          {data.map((item, index) => (
            <tr
              className={`border-b ${index % 2 === 0 ? "bg-grey" : "bg-light-gray"}`}
              key={index}
            >
              ...
              <td
                className={`py-4 px-6 text-center font-semibold rounded-lg ${item.V_status === 0 ? "text-red-700" : "text-green-700"}`}
              >
      ...
      {isModalOpen && (
        <div className="fixed left-0 inset-0 flex items-center justify-center bg-black bg-opacity-75 transition-opacity duration-300">
          <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md transition-transform transform scale-105">
            <button
              onClick={handleClose}
              className="absolute top-4 right-4 text-grey hover:text-dark-gray"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                x="0px"
            ...
            <h3 className="text-xl py-10 px-5 font-bold mb-4 text-primary">
              Approve or Cancel
            </h3>
            <div className="flex justify-around">
              <button
                onClick={handleApprove}
                className="py-2 px-4 bg-green-700 text-white font-semibold rounded-lg shadow hover:bg-green-700 transition"
              >
                Approve
              </button>
              <button
                onClick={handleCancel}
                className="py-2 px-4 bg-red-700 text-white font-semibold rounded-lg shadow hover:bg-red-700 transition"
              >
                Cancel
              </button>
            </div>
