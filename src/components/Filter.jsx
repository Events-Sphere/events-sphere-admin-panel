      ...
      {userCategory.length > 0 && (
        <div className="flex gap-3 ">
          ...
          {userCategory.map((role, index) => (
            ...
            //   </label>
            // </div>
<div id={role} className={`px-4 py-1 rounded-md text-white cursor-pointer items-center self-center  ${selectedRoles.includes(role) ? "bg-[#2F3645]": "bg-gradient-to-r from-[#6d879b] via-[#526D82] to-[#bed1df]"}`}
  ...
  {role }
      ...
      {user.length > 0 && (
        <div className="flex pt-5 md:p-0 gap-2 text-white">
          ...
          {user.map((user, index) => (
            <div className={`text-white  ${user=== 'true' ? " border-[1px] border-[#A2FF86]" : " border-[1px] border-[#E76161]"} px-2 py-2 flex align-middle rounded-md gap-1 `} key={index}>
